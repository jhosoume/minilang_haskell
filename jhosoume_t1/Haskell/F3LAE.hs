...
interp (Let x e1 e2) subs decs      = 
  let sub = interp e1 subs decs 
  in interp e2 ((x, sub):subs) decs
...
interp (LambdaApp e1 e2) subs decs  = 
    let (Closure x e fsubs) = (interp e1 subs decs)
        sub = (interp e2 subs decs)
    in interp e ((x,sub):fsubs) decs
interp (App n e) subs decs =
  let f = lookupFun n decs
    in case f of
        (Nothing) -> error "Function not declared"
        (Just (FunDec m a b)) -> 
            let sub = (interp e subs decs)
                in interp b ((a,sub):subs) decs
...
t7 = TestCase (assertEqual "Estatic" (NumValue  7) (interp (parse exp7) [] []))
...
t9 = TestCase (assertEqual "Closure" (NumValue 12) (interp (parse exp9) [] []))
...
t12 = TestCase (assertEqual "Scope2" (NumValue 7) (interp (parse exp12) [] []))
...
t14 = TestCase (assertEqual "Scope4" (NumValue 2) (interp (parse exp14) [] []))
